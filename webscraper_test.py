...
def extract_job_links(soup):
    """Extracts job links from the BeautifulSoup object."""
    return [a['href'] for a in soup.find_all('a', {'class': 'jcs-JobTitle'}, href=True)]

def extract_job_type(soup):
    """Extracts job type from the BeautifulSoup object."""
    job_type_element = soup.find('div', class_='css-1p3gyjy e1xnxm2i0')
    return job_type_element.text if job_type_element else None

def scrape_indeed_jobs(job_name, location, output_file):
    """Scrapes Indeed job listings, processes the data, and saves it to a CSV file."""
        ...
        for start in range(0, 50, 10):
            ...
            for i in range(len(job_titles)):
                data = {
                    ...
                    "Job Type": job_types[i] if i < len(job_types) else None,
...
# Example usage:
scrape_indeed_jobs(job_name='data science', location='Bengaluru', output_file='indeed_jobs')
