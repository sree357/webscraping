...
def extract_phone_number(html_content):
    """Extracts phone number from the HTML using a refined regular expression."""
    # Define a regular expression pattern to match phone numbers within the given HTML structure
    phone_pattern = re.compile(r'phone.*?0?(\d{10,11})', re.IGNORECASE | re.DOTALL)
    # Search for the pattern in the HTML content
    match = phone_pattern.search(html_content)
...
try:
    ...
    for company_link in company_links:
        ...
        # Extract phone number using the new method
        phone_number = extract_phone_number(company_page_source)
